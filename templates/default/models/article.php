    ...
    <div id="content" class="pt30 pb30">
        <div class="container">
            ...
            <div class="row">
				...
				<?php
				$result_article = $db->query('SELECT * FROM pm_article WHERE id_page = '.$page_id.' AND id != '.$article_id.' AND checked = 1 AND (publish_date IS NULL || publish_date <= '.time().') AND (unpublish_date IS NULL || unpublish_date > '.time().') AND lang = '.LANG_ID.' AND (show_langs IS NULL || show_langs = \'\' || show_langs REGEXP \'(^|,)'.LANG_ID.'(,|$)\') AND (hide_langs IS NULL || hide_langs = \'\' || hide_langs NOT REGEXP \'(^|,)'.LANG_ID.'(,|$)\') ORDER BY rand() LIMIT 3');
						...
						<div class="clearfix">
							...
							foreach($result_article as $i => $row){
								...
								if(isset($pages[$article_page])){
									...
									<article class="article-<?php echo $article_id; ?> col-sm-4 col-md-3 articleItem" itemscope itemtype="http://schema.org/Article">
